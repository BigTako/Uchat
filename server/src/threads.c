	...
	*/
	...
	switch(code_num)
	{
		...
		case TAKE_CURRENT_CHATS:
			...
			//get_db_data_table(sqlite3 * db, code template, int colums, int rows, int count, ...)
			table = get_db_data_table(param->db, db_query, 3, DB_ROWS_MAX, 3, CONVERSATIONS_TN, QUERY_DELIM, parts[1]);
			
			count_of_chats = null_str_arr_len(table);

			sprintf(query_buff, "%s%s%d", "W", QUERY_DELIM, count_of_chats);
